...
if(isset($_POST['themlhc'])){
    ...
    $sql_check = "SELECT COUNT(*) FROM usersv WHERE makhoa = '$id'";
    $result_check = mysqli_query($mysqli, $sql_check);
    ...
    if ($row[0] > 0) {
        $errors[] = "Không thể xóa Khoa này vì có liên quan trong Lớp hành chính.";
    }elseif($row2[0] > 0){
        $errors[] = "Không thể xóa Khoa này vì có liên quan trong học phần.";
    }else {  
        $sql_xoa = "DELETE FROM qlkhoa WHERE idkhoa = '$id'";
        if (!mysqli_query($mysqli, $sql_xoa)) {         
            $errors[] = "Không thể xóa khoa. Lỗi: " . mysqli_error($mysqli);
        } else {       
