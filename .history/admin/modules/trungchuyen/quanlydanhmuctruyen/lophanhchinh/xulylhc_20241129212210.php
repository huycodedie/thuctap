...
$id = mysqli_real_escape_string($mysqli, $_GET['idlhc']); // An toàn đầu vào
$errors = []; // Lỗi logic
$error = [];  

// Kiểm tra xem lớp hành chính có sinh viên hay không
...
if ($row[0] > 0) {
    ...
    // Xóa dữ liệu trong bảng viewusergv_khoa
    $sql_xoaview = "DELETE FROM viewusergv_khoa WHERE malhc='$id'";
    if (!mysqli_query($mysqli, $sql_xoaview)) {
        $error[] = "Không thể xóa dữ liệu liên quan trong viewusergv_khoa. Lỗi: " . mysqli_error($mysqli);
    }

    // Xóa dữ liệu trong bảng qllophanhchinh
    $sql_xoa = "DELETE FROM qllophanhchinh WHERE idlhc='$id'";
    if (!mysqli_query($mysqli, $sql_xoa)) {
        $error[] = "Không thể xóa lớp hành chính. Lỗi: " . mysqli_error($mysqli);
    } else {
