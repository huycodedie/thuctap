...
if(isset($_POST['themlhp'])){
    ...
    $checkmalhpQuery = "SELECT * FROM viewusergv_lhp WHERE malhp = '$malhp' AND magv = '$magv'";
    $result_check = mysqli_query($mysqli, $checkmalhpQuery);
    $row = mysqli_fetch_row($result_check);
    $checkdiemQuery = "SELECT * FROM bangdiem WHERE malhp = '$malhp'AND masv = '$masv'";
    $result_check2 = mysqli_query($mysqli, $checkdiemQuery);
    ...
    if($masv == 0){
        ...
        if($row2[0] > 0){
            $error[] = "Sinh viên đã có trong lớp học phần";
        }else{
            if ($row[0] > 0) {
               ...
               VALUE('".$mahp."','".$malhp."','".$magv."','".$masv."','".$ngaytao."')";
                ...
                $sql_themview = "INSERT INTO viewusergv_lhp(magv,malhp) 
                VALUE('".$magv."','".$malhp."')";
...
if (!empty($error)) {
    $_SESSION['error'] = $error;
    header("Location: ../../../../index.php?action=themsvl&malhp=$malhp");
}
