...
$sql_danhsach2 = "SELECT * FROM usersv 
                WHERE idsv ='$_GET[idsv]'"; 
$query_danhsach2 = mysqli_query($mysqli,$sql_danhsach2);

?>
<main id="main" class="main">
<?php          
       $trolai = mysqli_fetch_array($query_danhsach2);
        if($trolai){   
        ?>
    <div class="pagetitle">
    <h1>Danh sách điểm: <?php echo $trolai['usernamesv'] ?></h1>
    <nav>
    <ol class="breadcrumb">
      <li class="breadcrumb-item"><a href="index.php?action=khoa&idgv=<?php echo $user_id ?>">Home</a></li>
      <li class="breadcrumb-item"><a href="index.php?action=danh-sach-sv&idgv=<?php echo $user_id ?>&idlhc=<?php echo $tro['malhc'] ?>">danh sách</a></li>
