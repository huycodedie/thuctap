...
$sql_danhsach = "SELECT*FROM usergv g
                 ...
                 JOIN qllophocphan p ON p.malhp = b.malhp
                 JOIN usersv s ON s.idsv = b.masv
                WHERE b.magv = '$_GET[magv]'AND b.malhp = '$_GET[malhp]' AND ( s.usernamesv LIKE '%".$tukhoa."' OR b.masv LIKE '%".$tukhoa."%') ORDER BY b.madiem"; (b.masv LIKE ? OR b.malhp LIKE ?)        
$query_danhsach = mysqli_query($mysqli,$sql_danhsach);
$query_danhsach1 = mysqli_query($mysqli,$sql_danhsach);
$query_danhsach2 = mysqli_query($mysqli,$sql_danhsach);
...
$query_ten = mysqli_query($mysqli,$ten);
?>
<main id="main" class="main">

    <div class="pagetitle">
    <h1>Danh sách sinh viên lớp học phần: <?php $row = mysqli_fetch_array($query_ten); if($row){echo $row['tenlhp'];} ?></h1>
    <nav>
    <ol class="breadcrumb">
          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="search-bar">
                <form class="search-form  align-items-center" method="POST" action="index.php?action=timkiem&magv=<?php echo $user_id ?>&malhp=<?php echo $_GET['malhp'] ?>">
