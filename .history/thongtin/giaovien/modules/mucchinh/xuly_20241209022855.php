...
if(isset($_POST['gvthemdiem'])){
    ...
    $sql = "SELECT b.masv, s.usernamesv, p.tenlhp, b.quatrinh, b.cuoiky FROM bangdiem b JOIN usersv s ON s.idsv = b.masv JOIN qllophocphan p ON p.malhp = b.malhp";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $sheet->setCellValue('C' . $rowIndex, $row['usernames']);
        ...
        while ($row = $result->fetch_assoc()) {
            if (!isset($fileName) && !empty($row['tenlhp'])) {
                $fileName = "Dữ liệu lớp " . $row['tenlhp'] . " " . date('Y-m-d_H-i-s') . ".xlsx";
            }
            // Xử lý dữ liệu Excel như bình thường...
        }
