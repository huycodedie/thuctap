    ...
    if (isset($_FILES['file']) && $_FILES['file']['error'] == UPLOAD_ERR_OK) {
        ...
        if (in_array($fileType, $allowedTypes)) {
            ...
            foreach ($sheetData as $rowIndex => $row) {
                ...
                $checkquatrinh = "SELECT masv FROM bangdiem WHERE masv = '$masv'";
                $resultqt = $mysqli->query($checkSqlqua);

                $checkcuoiky = "SELECT masv FROM bangdiem WHERE masv = '$masv'";
                $resultck = $mysqli->query($checkSql);
